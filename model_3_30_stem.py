...
logging.basicConfig(
    filename='rekap_model/rekapmodel_3_30_stem.csv', filemode='w',format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
#logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
from gensim import corpora, models, similarities
from gensim.models import ldamodel
for i in range(30):
    lda_a = ldamodel.LdaModel(corpus, id2word=dictionary, num_topics=3, passes=15, alpha='auto')
    lda_a.save('model/stem/model_3_30_stem.model')
